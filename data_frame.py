...
class BeeData:
    ...
    def process_data(self, json_data, df):
        ...
        for entity in json_data.get("entities", []):
            ...
            try:
                ...
                if ts is None:
                    continue
                row["timestamp"] = datetime.datetime.fromtimestamp(ts / 1000, tz=ZoneInfo("Europe/Berlin"))
    ...
    def expand_by_hive(self, df):
        sensor_to_hive = {sensor: hive for hive, sensors in self.hive_sensor_map.items() for sensor in sensors}
        df["Hive_Name"] = df["Sensor_Name"].map(sensor_to_hive)
        df = df[df["Hive_Name"].notna()]
